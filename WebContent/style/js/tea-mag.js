...
function resetPwdRow(target){
	$.messager.confirm('确认','确定要重置该教师的密码为\"123456\"?',function(r){
		if (r){
			$('#user-table').datagrid('selectRow', getRowIndex(target));
			var row = $('#user-table').datagrid('getSelected');
			hyx(_rootPath+"/tea-mag/reset.do",{id:row.userid},
					function(data){
...
function deleterow(target){
	$.messager.confirm('确认','确定要删除该教师?',function(r){
		if (r){
			$('#user-table').datagrid('selectRow', getRowIndex(target));
			var row = $('#user-table').datagrid('getSelected');
			hyx(_rootPath+"/tea-mag/del.do",{id:row.userid},
					function(data){
...
function saverow(target){
	...
	var row = $('#user-table').datagrid('getSelected');
	hyx(_rootPath+"/tea-mag/add.do",{id:row.userid,loginName:row.loginName,realName:row.realName,clzName:row.clzName},
			function(data){
		if(data.s) {
...
$('#user-table').datagrid({
	...
	columns:[[
		...
		{field:'clzName', title:'任课的实验室编号', width:200, align:'center',
			editor:{
				type:'text',
				options:{
					required:true
